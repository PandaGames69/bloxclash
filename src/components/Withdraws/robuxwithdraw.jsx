        ...
        <>
            <div class='robux-deposit-container'>
                ...
                <button class='bevel-gold deposit' onClick={async () => {
                    let res = await authedAPI('/trading/robux/withdraw', 'POST', JSON.stringify({
                        amount: robux(),
                    }), true)
                ...
                <Show when={!txsResource.loading} fallback={<Loader/>}>
                    <For each={txs()?.filter(tx => tx.operation === 'withdraw')}>{(tx, index) =>
